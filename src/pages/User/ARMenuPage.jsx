...
const ARMenuPage = () => {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}><span className="material-symbols-outlined" style={s.headerIcon}>view_in_ar</span><h1 style={s.heading}>Thuc don tuong tac</h1><p style={s.subtitle}>Trai nghiem mon an bang AR</p></div>
      <div style={s.viewfinder}><div style={s.cornerTL} /><div style={s.cornerTR} /><div style={s.cornerBL} /><div style={s.cornerBR} /><span className="material-symbols-outlined" style={s.crosshair}>center_focus_strong</span><p style={s.viewfinderText}>Huong camera vao menu</p></div>
        ...
        {dishes.map((dish) => (
          <div key={dish.id} style={s.dishCard}>
            ...
            <div style={s.dishBody}>
              ...
              <div style={s.dishMeta}><span style={s.dishPrice}>{dish.price.toLocaleString()}d</span><span style={s.dishCal}><span className="material-symbols-outlined" style={s.calIcon}>local_fire_department</span>{dish.cal} kcal</span></div>
              <div style={s.stars}>{renderStars(dish.rating)}</div>
              <div style={s.btnRow}><button style={s.btn3D}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>view_in_ar</span>Xem 3D</button><button style={s.btnOrder}>Dat mon</button></div>
            </div>
          </div>
        ))}
      </div>
      <div style={s.nutritionCard} onClick={() => setNutritionOpen(!nutritionOpen)}>
        <div style={s.nutritionHeader}><div style={s.nutritionTitle}><span className="material-symbols-outlined" style={s.nutritionIcon}>monitoring</span>Thong tin dinh duong</div><span className="material-symbols-outlined" style={{ ...s.expandIcon, transform: nutritionOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>expand_more</span></div>
      ...
      <div style={s.shareCard}><span className="material-symbols-outlined" style={s.shareIcon}>photo_camera</span><p style={s.shareText}>Chup & chia se tren GOMET</p><button style={s.shareBtn}>Chup anh</button></div>
    </div>
  );
};
