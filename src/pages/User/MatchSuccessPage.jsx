...
const MatchSuccessPage = () => {
  ...
  const matchData = { user1: { name: 'Bạn', avatar: '' }, user2: { name: 'Minh Anh', avatar: '' }, percentage: 92, sharedInterests: ['Cà phê', 'Du lịch', 'Âm nhạc'] };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(19,19,19,0.95)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, fontFamily: 'var(--font-body)' }}>
      ...
      <div style={{ background: '#1C1B1B', borderRadius: '1.5rem', padding: '40px 32px', maxWidth: 380, width: '90%', textAlign: 'center', animation: 'fadeInUp 0.6s ease-out', position: 'relative', zIndex: 2 }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-headline)', fontSize: 22, fontWeight: 800, color: '#FDF9F3', marginBottom: 8 }}>Chúc mừng! Các bạn đã ghép đôi! 🎉</h1>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#2A2A2A', color: '#FFB59E', borderRadius: '9999px', padding: '6px 16px', fontSize: 14, fontWeight: 600, marginBottom: 24 }}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 18 }}>monitoring</span>{matchData.percentage}% tương thích</div>
        <div style={{ marginBottom: 28 }}><p style={{ fontSize: 13, color: '#E6BEB2', marginBottom: 10, fontWeight: 500 }}>Sở thích chung</p><div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>{matchData.sharedInterests.map((interest, i) => (<span key={i} style={{ background: '#2A2A2A', color: '#FFB59E', borderRadius: '9999px', padding: '6px 14px', fontSize: 13, fontWeight: 500 }}>{interest}</span>))}</div></div>
        <button onClick={() => navigate('/app/chat')} style={{ width: '100%', padding: '14px 0', background: 'linear-gradient(135deg, #FFB59E, #FF571A)', color: '#3A0B00', border: 'none', borderRadius: '1.5rem', fontSize: 15, fontWeight: 700, fontFamily: 'var(--font-headline)', cursor: 'pointer', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 20 }}>send</span>Gửi tin nhắn</button>
        <button onClick={() => navigate('/app')} style={{ width: '100%', padding: '14px 0', background: 'transparent', color: '#FFB59E', border: 'none', borderRadius: '1.5rem', fontSize: 15, fontWeight: 600, fontFamily: 'var(--font-headline)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 20 }}>explore</span>Tiếp tục khám phá</button>
      </div>
    </div>
  );
};
