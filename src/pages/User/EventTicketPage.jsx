...
const EventTicketPage = () => {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.actions}><button onClick={() => alert('Đã lưu vé vào thiết bị!')} style={s.actionBtn}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 20 }}>download</span>Lưu vé</button><button onClick={() => navigator.share ? navigator.share({ title: ticketData.eventName, url: window.location.href }) : alert('Đã sao chép liên kết vé!')} style={s.actionBtn}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 20 }}>share</span>Chia sẻ vé</button></div>
