  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#131313', fontFamily: 'var(--font-body)' }}>
      <div style={{ backgroundColor: '#1C1B1B', padding: '16px 20px', display: 'flex', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ flex: 1, textAlign: 'center' }}><h1 style={{ fontFamily: 'var(--font-headline)', fontSize: 18, fontWeight: 700, color: '#FDF9F3', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><span className="material-symbols-outlined" style={{ fontSize: 22, color: '#FFB59E' }}>how_to_reg</span>Dang ky su kien</h1></div>
        <div style={{ width: 24 }} />
      </div>
      <div style={{ maxWidth: 540, margin: '0 auto', padding: '20px 16px 100px' }}>
        <div style={{ background: '#1C1B1B', borderRadius: '1.5rem', padding: '18px', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{[{ icon: 'event', text: event.date }, { icon: 'location_on', text: event.venue }, { icon: 'payments', text: `Tu ${event.price}` }].map((item, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#E6BEB2' }}><span className="material-symbols-outlined" style={{ fontSize: 18, color: '#FFB59E' }}>{item.icon}</span>{item.text}</div>))}</div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: '#FF571A', color: '#3A0B00', borderRadius: '9999px', padding: '4px 12px', fontSize: 12, fontWeight: 600, marginTop: 10 }}><span className="material-symbols-outlined" style={{ fontSize: 14 }}>group</span>Con {event.spotsLeft} cho trong</div>
        </div>
