...
const AlphaTestPage = () => {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}><button style={s.backBtn} onClick={() => navigate(-1)}><span className="material-symbols-outlined">arrow_back</span></button><h1 style={s.pageTitle}>GOMET Alpha</h1><span className="material-symbols-outlined" style={s.scienceIcon}>science</span></div>
      ...
      {features.map(f => (<div key={f.id} style={s.featureCard}><div style={s.featureHeader}><span style={s.featureName}>{f.name}</span><span style={s.versionBadge}>{f.version}</span></div><p style={s.featureDesc}>{f.desc}</p><div style={s.featureFooter}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><div style={s.statusBadge(f.status)}><div style={s.statusDot(f.status)} />{f.status === 'testing' ? 'Dang test' : 'Hoan thanh'}</div><span style={s.feedbackCount}><span className="material-symbols-outlined" style={{ fontSize: '16px' }}>chat_bubble</span>{f.feedbackCount} phan hoi</span></div>{f.tested ? (<button style={s.triedBtn} disabled>Da thu</button>) : (<button style={s.tryBtn}>Thu ngay</button>)}</div></div>))}
      ...
      <div style={s.feedbackSection}>
        ...
        <div style={s.bugToggle}><div style={s.toggleTrack(isBug)} onClick={() => setIsBug(!isBug)}><div style={s.toggleThumb(isBug)} /></div><span style={s.bugLabel}>Day la loi?</span>{isBug && (<span className="material-symbols-outlined" style={{ color: '#FF571A', fontSize: '20px' }}>bug_report</span>)}</div>
