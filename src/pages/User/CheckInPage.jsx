...
const CheckInPage = () => {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}><button style={s.backBtn} onClick={() => navigate(-1)}><span aria-hidden="true" className="material-symbols-outlined">arrow_back</span></button><span aria-hidden="true" className="material-symbols-outlined" style={s.headerIcon}>qr_code_scanner</span><h1 style={s.headerTitle}>Check-in sự kiện</h1></div>
      {!checkedIn ? (
        <>
          <div style={s.scannerContainer}><div style={s.scannerBox}><div style={s.corner(0, 0, null, null)} /><div style={s.corner(0, null, 0, null)} /><div style={s.corner(null, 0, null, 0)} /><div style={s.corner(null, null, 0, 0)} /><div style={s.scanLine} /></div><p style={s.scanInstruction}>Quét mã QR để check-in</p></div>
          <div style={s.divider}><div style={s.dividerLine} /><span style={s.dividerText}>Hoặc nhập mã</span><div style={s.dividerLine} /></div>
          <div style={s.manualSection}><input style={s.input} placeholder="Nhập mã check-in..." value={manualCode} onChange={(e) => setManualCode(e.target.value)} /><button style={s.confirmBtn} onClick={handleCheckIn}>Xác nhận</button></div>
        </>
      ) : (
        <>
          <div style={s.successContainer}><div style={s.checkCircle}><span aria-hidden="true" className="material-symbols-outlined" style={s.checkIcon}>check</span></div><h2 style={s.successTitle}>Check-in thành công!</h2><p style={s.successDetail}>DineDate: Dinner & Drinks</p><p style={s.successDetail}>Thời gian: 19:32 - 22/03/2026</p><p style={s.successDetail}>Bàn: A5 - Tầng 2</p></div>
          <div style={s.eventCard}><div style={s.eventCardTitle}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: '18px' }}>info</span>Thông tin sự kiện</div><div style={s.eventRow}><span aria-hidden="true" className="material-symbols-outlined" style={s.eventRowIcon}>event</span><span style={s.eventRowText}>DineDate: Dinner & Drinks</span></div><div style={s.eventRow}><span aria-hidden="true" className="material-symbols-outlined" style={s.eventRowIcon}>location_on</span><span style={s.eventRowText}>Lầu Tầng 5, 72 Nguyễn Huệ, Q1</span></div><div style={s.eventRow}><span aria-hidden="true" className="material-symbols-outlined" style={s.eventRowIcon}>schedule</span><span style={s.eventRowText}>19:00 - 22:00, 22/03/2026</span></div><button style={s.viewDetailLink} onClick={() => navigate('/app/event/1')}>Xem chi tiết →</button></div>
        </>
      )}
    </div>
  );
