...
const ManageEvents = () => {
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.topRow}><div style={styles.header}><span className="material-symbols-outlined" style={styles.headerIcon}>event</span><h1 style={styles.title}>Quan ly su kien</h1></div><button style={styles.createBtn} onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.03)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; }}><span className="material-symbols-outlined" style={{ fontSize: 20 }}>add</span>Tao su kien moi</button></div>
      ...
      {filtered.length === 0 ? (<div style={styles.emptyState}><span className="material-symbols-outlined" style={styles.emptyIcon}>event_busy</span><div style={{ fontFamily: 'var(--font-headline)', fontSize: 18, fontWeight: 600, marginBottom: 8 }}>Khong co su kien nao</div><div style={{ fontSize: 14 }}>Khong tim thay su kien trong muc nay.</div></div>) : (
        filtered.map((event, idx) => (
          <div key={event.id} style={styles.eventCard}>
            <div style={styles.eventImage(gradients[idx % gradients.length])}><span className="material-symbols-outlined" style={styles.eventImageIcon}>celebration</span></div>
            <div style={styles.eventBody}>
              <div><div style={styles.eventName}>{event.name}</div><div style={styles.eventMeta}><span className="material-symbols-outlined" style={styles.metaIcon}>calendar_today</span>{event.date} | {event.time}</div><div style={styles.eventMeta}><span className="material-symbols-outlined" style={styles.metaIcon}>location_on</span>{event.venue}</div></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginTop: 12, marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span className="material-symbols-outlined" style={styles.metaIcon}>group</span><div style={{ display: 'flex', marginLeft: 4 }}>{event.avatars.map((a, i) => (<div key={i} style={styles.avatar(i)}>{a}</div>))}</div><span style={styles.attendeeCount}>{event.attendees}/{event.maxAttendees}</span></div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}><button style={styles.actionBtn('default')}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>edit</span>Chinh sua</button><button style={styles.actionBtn('default')}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>visibility</span>Xem chi tiet</button>{event.status !== 'ended' && (<button style={styles.actionBtn('error')}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>cancel</span>Huy</button>)}</div>
            </div>
          </div>
        ))
      )}
