...
const PartnerDashboard = () => {
  ...
  return (
    <div style={st.page}>
      <div style={st.header}><span aria-hidden="true" className="material-symbols-outlined" style={st.headerIcon}>storefront</span><h1 style={st.title}>Bang dieu khien doi tac</h1></div>
      ...
      <div style={st.twoCol}>
        <div style={st.section}><h2 style={st.sectionTitle}>Dat cho sap toi</h2>{bookings.map((b) => (<div key={b.id} style={st.bookingItem}><div style={{ display: 'flex', alignItems: 'center', gap: 16 }}><div style={st.bookingTime}>{b.time}</div><div><div style={st.bookingGuest}>{b.guest}</div><div style={st.bookingParty}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 16, color: '#666460' }}>group</span>{b.party} khach</div></div></div><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span style={b.status === 'confirmed' ? st.badgeConfirmed : st.badgePending}>{b.status === 'confirmed' ? 'Xac nhan' : 'Cho duyet'}</span><button style={{ ...st.actionBtn, ...st.acceptBtn }} title="Xac nhan"><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 18 }}>check</span></button><button style={{ ...st.actionBtn, ...st.rejectBtn }} title="Tu choi"><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 18 }}>close</span></button></div></div>))}</div>
