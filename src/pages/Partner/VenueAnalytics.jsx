...
const VenueAnalytics = () => {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.section}><div style={s.sectionTitle}><span aria-hidden="true" className="material-symbols-outlined" style={{ color: '#FFB59E' }}>trending_up</span>Danh gia theo thoi gian</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12 }}>{ratingTrend.map((t, i) => (<div key={i} style={s.trendCard}><div style={{ fontSize: 13, color: '#E6BEB2', marginBottom: 8 }}>{t.month}</div><div style={{ fontFamily: 'var(--font-headline)', fontSize: 28, fontWeight: 800, color: '#FFB59E' }}>{t.rating}</div><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 20, color: t.trend === 'up' ? '#117500' : t.trend === 'down' ? '#FF571A' : '#E6BEB2', verticalAlign: 'middle' }}>{t.trend === 'up' ? 'arrow_upward' : t.trend === 'down' ? 'arrow_downward' : 'remove'}</span></div>))}</div></div>
    </div>
  );
};
