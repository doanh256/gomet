...
const VenueAnalytics = () => {
  ...
  const periods = ['7 ngày', '30 ngày', '3 tháng', '1 năm'];
  const stats = [{ label: 'Tổng lượt xem', value: '12,450', icon: 'visibility', change: '+12%' }, { label: 'Tỷ lệ đặt chỗ', value: '68%', icon: 'event_available', change: '+5%' }, { label: 'Đánh giá TB', value: '4.7', icon: 'star', change: '+0.2' }, { label: 'Khách quay lại', value: '42%', icon: 'replay', change: '+8%' }];
  ...
  const peakHours = { rows: ['Sáng', 'Trưa', 'Tối'], cols: ['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'], data: [[0.2, 0.15, 0.25, 0.2, 0.3, 0.5, 0.45], [0.5, 0.55, 0.5, 0.6, 0.7, 0.85, 0.8], [0.7, 0.75, 0.65, 0.8, 0.95, 1.0, 0.9]] };
  const sources = [{ label: 'GOMET App', percent: 65, color: '#FFB59E' }, { label: 'Tìm kiếm', percent: 20, color: '#FFD54F' }, { label: 'Giới thiệu', percent: 15, color: '#E6BEB2' }];
  const ratingTrend = [{ month: 'Tháng 12', rating: 4.5, trend: 'up' }, { month: 'Tháng 1', rating: 4.6, trend: 'up' }, { month: 'Tháng 2', rating: 4.7, trend: 'up' }, { month: 'Tháng 3', rating: 4.7, trend: 'same' }];
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 32, background: 'linear-gradient(135deg, #FFB59E, #FF571A)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>analytics</span><h1 style={{ fontFamily: 'var(--font-headline)', fontSize: 28, fontWeight: 800 }}>Phân tích hiệu suất</h1></div>
      <p style={{ color: '#E6BEB2', fontSize: 14, marginBottom: 24 }}>Theo dõi và phân tích hiệu suất địa điểm của bạn</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 32 }}>{stats.map((st, i) => (<div key={i} style={s.statCard}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 28, color: '#FFB59E', background: '#2A2A2A', borderRadius: '1.5rem', padding: 8 }}>{st.icon}</span><div><div style={{ fontFamily: 'var(--font-headline)', fontSize: 24, fontWeight: 800 }}>{st.value}</div><div style={{ fontSize: 13, color: '#E6BEB2', marginTop: 2 }}>{st.label}</div><div style={{ fontSize: 12, color: '#117500', fontWeight: 600, marginTop: 2 }}>{st.change}</div></div></div>))}</div>
      <div style={s.section}><div style={s.sectionTitle}><span aria-hidden="true" className="material-symbols-outlined" style={{ color: '#FFB59E' }}>bar_chart</span>Biểu đồ lượt xem</div><div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, height: 200, padding: '0 8px' }}>{weeklyViews.map((d, i) => (<div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, position: 'relative' }}><div style={s.bar((d.value / maxView) * 100, hoveredBar === i)} onMouseEnter={() => setHoveredBar(i)} onMouseLeave={() => setHoveredBar(null)}>{hoveredBar === i && (<div style={s.barTooltip}>{d.value.toLocaleString()}</div>)}</div><span style={{ fontSize: 12, color: '#E6BEB2', fontWeight: 500 }}>{d.day}</span></div>))}</div></div>
      <div style={s.section}><div style={s.sectionTitle}><span aria-hidden="true" className="material-symbols-outlined" style={{ color: '#FFB59E' }}>schedule</span>Giờ cao điểm</div><div style={{ display: 'grid', gridTemplateColumns: '60px repeat(7, 1fr)', gap: 4 }}><div />{peakHours.cols.map(c => (<div key={c} style={{ fontSize: 12, fontWeight: 600, color: '#E6BEB2', textAlign: 'center' }}>{c}</div>))}{peakHours.rows.map((row, ri) => (<React.Fragment key={row}><div style={{ fontSize: 12, fontWeight: 600, color: '#E6BEB2', display: 'flex', alignItems: 'center' }}>{row}</div>{peakHours.data[ri].map((val, ci) => (<div key={ci} style={s.heatCell(val)}>{Math.round(val * 100)}%</div>))}</React.Fragment>))}</div></div>
      <div style={s.section}><div style={s.sectionTitle}><span aria-hidden="true" className="material-symbols-outlined" style={{ color: '#FFB59E' }}>pie_chart</span>Nguồn khách</div>{sources.map((src, i) => (<div key={i} style={{ marginBottom: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 14, fontWeight: 500 }}><span>{src.label}</span><span style={{ fontWeight: 700 }}>{src.percent}%</span></div><div style={{ height: 12, background: '#2A2A2A', borderRadius: '9999px', overflow: 'hidden' }}><div style={{ height: '100%', width: `${src.percent}%`, background: src.color, borderRadius: '9999px', transition: 'width 0.6s ease' }} /></div></div>))}</div>
      <div style={s.section}><div style={s.sectionTitle}><span aria-hidden="true" className="material-symbols-outlined" style={{ color: '#FFB59E' }}>trending_up</span>Đánh giá theo thời gian</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12 }}>{ratingTrend.map((t, i) => (<div key={i} style={s.trendCard}><div style={{ fontSize: 13, color: '#E6BEB2', marginBottom: 8 }}>{t.month}</div><div style={{ fontFamily: 'var(--font-headline)', fontSize: 28, fontWeight: 800, color: '#FFB59E' }}>{t.rating}</div><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: 20, color: t.trend === 'up' ? '#117500' : t.trend === 'down' ? '#FF571A' : '#E6BEB2', verticalAlign: 'middle' }}>{t.trend === 'up' ? 'arrow_upward' : t.trend === 'down' ? 'arrow_downward' : 'remove'}</span></div>))}</div></div>
    </div>
  );
};
