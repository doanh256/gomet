...
const RevenueReport = () => {
  ...
  const summary = [
    { label: 'Tong doanh thu', value: '45.2M VND', icon: 'account_balance', change: '+18%', changeColor: '#117500', changeIcon: 'trending_up' },
    { label: 'So giao dich', value: '234', icon: 'receipt_long', change: '+12%', changeColor: '#117500', changeIcon: 'trending_up' },
    { label: 'Gia tri TB', value: '193k VND', icon: 'analytics', change: '+5%', changeColor: '#117500', changeIcon: 'trending_up' },
  ];
  ...
  const distribution = [{ label: 'Dat cho', percent: 60, color: '#FFB59E' }, { label: 'Su kien', percent: 25, color: '#FFD54F' }, { label: 'Khuyen mai', percent: 15, color: '#E6BEB2' }];
  const getTypeChipStyle = (type) => {
    if (type === 'Dat cho') return { background: 'rgba(255,181,158,0.15)', color: '#FFB59E' };
    if (type === 'Su kien') return { background: 'rgba(255,213,79,0.15)', color: '#FFD54F' };
    if (type === 'Khuyen mai') return { background: 'rgba(17,117,0,0.15)', color: '#117500' };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#131313', fontFamily: 'var(--font-body)', color: '#FDF9F3', padding: '24px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}><span className="material-symbols-outlined" style={{ fontSize: 32, background: 'linear-gradient(135deg, #FFB59E, #FF571A)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>account_balance</span><h1 style={{ fontFamily: 'var(--font-headline)', fontSize: 28, fontWeight: 800 }}>Bao cao doanh thu</h1></div>
      <p style={{ color: '#E6BEB2', fontSize: 14, marginBottom: 24 }}>Theo doi doanh thu va giao dich cua dia diem</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 32 }}>
        {summary.map((st, i) => (<div key={i} style={{ background: '#1C1B1B', borderRadius: '1.5rem', padding: '24px' }}><span className="material-symbols-outlined" style={{ fontSize: 28, color: '#FFB59E', marginBottom: 8 }}>{st.icon}</span><div style={{ fontFamily: 'var(--font-headline)', fontSize: 28, fontWeight: 800, marginBottom: 4 }}>{st.value}</div><div style={{ fontSize: 13, color: '#E6BEB2', marginBottom: 8 }}>{st.label}</div><div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 13, fontWeight: 600, color: st.changeColor }}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>{st.changeIcon}</span>{st.change}</div></div>))}
      </div>
      <div style={{ background: '#1C1B1B', borderRadius: '1.5rem', padding: '24px', marginBottom: 24 }}>
        <div style={{ fontFamily: 'var(--font-headline)', fontSize: 18, fontWeight: 700, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}><span className="material-symbols-outlined" style={{ color: '#FFB59E' }}>bar_chart</span>Doanh thu theo ngay</div>
      ...
      <div style={{ background: '#1C1B1B', borderRadius: '1.5rem', padding: '24px', marginBottom: 24 }}>
        <div style={{ fontFamily: 'var(--font-headline)', fontSize: 18, fontWeight: 700, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}><span className="material-symbols-outlined" style={{ color: '#FFB59E' }}>list_alt</span>Chi tiet giao dich</div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
        <button style={{ background: '#2A2A2A', border: 'none', borderRadius: '9999px', padding: '12px 24px', fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-body)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, color: '#FDF9F3' }}><span className="material-symbols-outlined" style={{ fontSize: 20 }}>picture_as_pdf</span>Xuat PDF</button>
        <button style={{ background: '#2A2A2A', border: 'none', borderRadius: '9999px', padding: '12px 24px', fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-body)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, color: '#FDF9F3' }}><span className="material-symbols-outlined" style={{ fontSize: 20 }}>table_chart</span>Xuat Excel</button>
      </div>
      <div style={{ background: '#1C1B1B', borderRadius: '1.5rem', padding: '24px' }}>
        <div style={{ fontFamily: 'var(--font-headline)', fontSize: 18, fontWeight: 700, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}><span className="material-symbols-outlined" style={{ color: '#FFB59E' }}>donut_large</span>Phan bo doanh thu</div>
