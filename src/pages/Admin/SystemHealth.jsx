...
const SystemHealth = () => {
  ...
  const maintenance = { title: 'Bảo trì hệ thống định kỳ', date: '30/03/2026', time: '02:00 - 04:00', desc: 'Nâng cấp database engine và tối ưu hiệu suất server. Dự kiến downtime 30 phút.' };
  ...
  return (
    <div style={s.page}>
      <div style={s.header}><span aria-hidden="true" className="material-symbols-outlined" style={s.headerIcon}>monitor_heart</span><h1 style={s.title}>Tình trạng hệ thống</h1></div>
      <div style={s.overallCard}><div style={s.overallCircle(overallOk)}><span aria-hidden="true" className="material-symbols-outlined" style={s.overallIcon(overallOk)}>{overallOk ? 'check_circle' : 'error'}</span></div><div><div style={s.overallText(overallOk)}>{overallOk ? 'HOẠT ĐỘNG TỐT' : 'CÓ VẤN ĐỀ'}</div><div style={s.overallSub}>Tất cả dịch vụ đang hoạt động bình thường. Cập nhật lúc 14:35 hôm nay.</div></div></div>
      <div style={s.section}><h2 style={s.sectionTitle}>Trạng thái dịch vụ</h2><div style={s.serviceGrid}>{services.map((sv, i) => (<div key={i} style={s.serviceCard}><div style={s.serviceDot(sv.status)} /><div style={s.serviceName}>{sv.name}</div><div style={s.serviceUptime}>{sv.uptime}%</div></div>))}</div></div>
      <div style={s.section}><h2 style={s.sectionTitle}>Hiệu suất</h2><div style={s.metricGrid}>{metrics.map((m, i) => { const offset = circumference - (m.value / 100) * circumference; return (<div key={i} style={s.metricCard}><div style={{ position: 'relative', width: circleSize, height: circleSize }}><svg width={circleSize} height={circleSize} style={{ transform: 'rotate(-90deg)' }}><circle cx={circleSize / 2} cy={circleSize / 2} r={radius} fill="none" stroke="#2A2A2A" strokeWidth={strokeWidth} /><circle cx={circleSize / 2} cy={circleSize / 2} r={radius} fill="none" stroke={m.color} strokeWidth={strokeWidth} strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" style={{ transition: 'stroke-dashoffset 0.6s ease' }} /></svg><div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', ...s.metricValue, color: m.color }}>{m.value}%</div></div><div style={s.metricLabel}>{m.label}</div></div>); })}</div></div>
      <div style={s.section}><h2 style={s.sectionTitle}>Nhật ký lỗi gần đây</h2><div style={s.logCard}>{errors.map((err, i) => (<div key={i} style={s.logRow}><div style={s.logTime}>{err.time}</div><span style={s.sevChip(err.severity)}>{err.severity}</span><div style={s.logMsg}>{err.msg}</div></div>))}</div></div>
      <div style={s.section}><h2 style={s.sectionTitle}>Bảo trì</h2><div style={s.maintCard}><span aria-hidden="true" className="material-symbols-outlined" style={s.maintIcon}>calendar_month</span><div><div style={s.maintTitle}>{maintenance.title}</div><div style={s.maintDate}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: '16px' }}>event</span>{maintenance.date} | {maintenance.time}</div><div style={s.maintDesc}>{maintenance.desc}</div><span style={s.maintBadge}>Đã lên lịch</span></div></div></div>
    </div>
  );
};
