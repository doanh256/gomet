...
const SystemHealth = () => {
  ...
  return (
    <div style={s.page}>
      <div style={s.header}><span aria-hidden="true" className="material-symbols-outlined" style={s.headerIcon}>monitor_heart</span><h1 style={s.title}>Tinh trang he thong</h1></div>
      <div style={s.overallCard}><div style={s.overallCircle(overallOk)}><span aria-hidden="true" className="material-symbols-outlined" style={s.overallIcon(overallOk)}>{overallOk ? 'check_circle' : 'error'}</span></div><div><div style={s.overallText(overallOk)}>{overallOk ? 'HOAT DONG TOT' : 'CO VAN DE'}</div><div style={s.overallSub}>Tat ca dich vu dang hoat dong binh thuong. Cap nhat luc 14:35 hom nay.</div></div></div>
      ...
      <div style={s.section}><h2 style={s.sectionTitle}>Bao tri</h2><div style={s.maintCard}><span aria-hidden="true" className="material-symbols-outlined" style={s.maintIcon}>calendar_month</span><div><div style={s.maintTitle}>{maintenance.title}</div><div style={s.maintDate}><span aria-hidden="true" className="material-symbols-outlined" style={{ fontSize: '16px' }}>event</span>{maintenance.date} | {maintenance.time}</div><div style={s.maintDesc}>{maintenance.desc}</div><span style={s.maintBadge}>Da len lich</span></div></div></div>
    </div>
  );
};
